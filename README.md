...
| `npm run test`            | 테스트 실행           |
| `npm run test:e2e`        | Chromium E2E 실행     |
| `npm run test:e2e:repeat` | Chromium E2E 2회 반복 |
| `npm run db:validate`     | Prisma schema 검사    |
| `npm run db:generate`     | Prisma Client 생성    |
| `npm run db:studio`       | Prisma Studio 실행    |

실제 브라우저 인증·개인화 테스트는 운영 DB가 아닌 localhost의 전용 M3 DB만 허용한다. 환경 설정, 인증 우회 차단 조건과 CI artifact 정책은 [M3-T13 브라우저 E2E 문서](docs/m3-t13-browser-e2e.md)를 따른다.
