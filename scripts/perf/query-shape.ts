...
const prisma = new PrismaClient({
  adapter,
  log: args.sqlEvents ? [{ emit: "event", level: "query" }] : []
});
const sqlLog: PerfQueryShapeSqlLog | undefined = args.sqlEvents
  ? { events: [] }
  : undefined;
...
try {
  const report = await runPerfQueryShape(
    toPerfQueryShapeDbClient(prisma),
    {
  ...
  console.error("error: failed to run Prisma query-shape inspection");
  console.error(`error: ${errorMessage(error)}`);
  console.error(
    "Check DATABASE_URL, migrations, seed import state, and CLI options. The script is read-only and does not mutate DB rows."
  );
  process.exitCode = 1;
} finally {
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--no-sql-events") {
      parsed.sqlEvents = false;
      continue;
    }
...
function printHelpAndExit(exitCode = 0): never {
  console.log(`Usage:
  npm run perf:query-shape -- [options]
...
Options:
  ...
  --no-sql-events             Disable Prisma query event logging and only record client method counts
  --help                      Show this help
`);
...
function createPgPoolConfig() {
  try {
    return createSeedPgPoolConfig("perf:query-shape");
  } catch (error) {
    console.error(`error: ${errorMessage(error)}`);
    process.exit(1);
  }
...
function toPerfQueryShapeDbClient(
